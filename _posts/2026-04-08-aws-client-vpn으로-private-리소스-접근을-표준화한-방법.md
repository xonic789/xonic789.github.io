...
구성 자체는 비교적 단순했다.  
다만 이 글에서 더 중요하게 보고 싶은 지점은 네트워크 토폴로지보다, **누가 신뢰를 보증하고 그 신뢰를 어떻게 회수하는가**에 가깝다.
...
이 구조에서는 누가 정당한 사용자인지, 지금 접속한 엔드포인트가 정말 우리가 신뢰하는 서버인지, 이미 회수된 자격 증명은 아닌지를 모두 **인증서와 서명 검증**으로 판단한다.
...
즉 이 구조는 단순히 암호화된 통신 채널을 여는 것이 아니라,  
**신뢰의 발급, 검증, 회수까지를 인증서 모델로 일관되게 다루는 방식**에 가깝다.
...
즉 이 에러는 대개 전송 계층 자체보다,  
**개인 키와 인증서 체인이 정상적으로 검증되는가**를 먼저 의심하는 것이 맞았다.
