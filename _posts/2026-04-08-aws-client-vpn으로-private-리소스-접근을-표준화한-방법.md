...
반대로 코드로 관리하면 변경 이력과 의도가 함께 남는다.

넷째, Split Tunnel 구성이 가능했다.  
일반 인터넷 트래픽은 로컬 경로를 유지하고 내부 리소스 접근에 필요한 경로만 VPN을 통해 전달하는 구조가 된다는 점도 장점이었다.

## 우리가 구성한 구조

구성 자체는 비교적 단순했다.  
다만 이 글에서 더 중요하게 보고 싶은 지점은 네트워크 토폴로지보다 **누가 신뢰를 보증하고 그 신뢰를 어떻게 회수하는가** 쪽이다.

![AWS Client VPN 아키텍처 다이어그램](/assets/img/posts/2026/aws-client-vpn-private-access-architecture.svg){: w="1100" }
_네트워크 구조는 배경 문맥이고, 실제 통제의 핵심은 뒤에서 설명할 인증서 기반 신뢰 모델이다._
...
이렇게 하면 접근 경로를 문서화하기 쉬워지고 보안 정책도 한 방향으로 정리된다.
...
표면적으로 보면 “VPN 클라이언트가 접속한다” 정도로 보이지만 실제 핵심은 인증서 체계다.  
이 구조에서는 누가 정당한 사용자인지, 지금 접속한 엔드포인트가 정말 우리가 신뢰하는 서버인지, 이미 회수된 자격 증명은 아닌지를 모두 인증서와 서명 검증으로 판단한다.

![AWS Client VPN 인증서 기반 동작 원리](/assets/img/posts/2026/aws-client-vpn-certificate-flow.svg){: w="1100" }
_CA의 서명이 신뢰의 기준이 되고, 서버와 사용자는 각자 개인 키 소유를 증명하며 폐기된 인증서는 CRL로 차단된다._
...
- 서버 인증서는 “지금 접속한 Endpoint가 우리가 신뢰하는 대상인가”를 증명한다
- 클라이언트 인증서는 “이 사용자가 CA가 발급한 자격을 실제로 소유하고 있는가”를 증명한다
- CA 서명은 어떤 인증서를 신뢰할지 판단하는 기준점이 된다
- CRL은 이미 발급된 인증서라도 이후에 신뢰를 철회할 수 있게 만든다

단순히 암호화된 통신 채널을 여는 것이 아니라,  
**신뢰의 발급, 검증, 회수까지를 인증서 모델로 일관되게 다루는 방식**인 셈이다.

통신 설정 자체는 상대적으로 부가적인 영역이었다.  
실제 운영에서는 아래 정도만 맞추면 됐다.
...
연결 품질과 사용자 경험에는 중요했지만  
보안 모델의 중심은 어디까지나 인증서와 개인 키, 그리고 폐기 목록 관리 쪽에 있었다.
...
- 운영 정책을 문서화하기 좋음
- 이미 배포된 `.ovpn` 파일이 남아 있어도 신뢰 자체를 끊을 수 있음
...
이 에러는 대개 전송 계층 자체보다,  
개인 키와 인증서 체인이 정상적으로 검증되는가를 먼저 의심하는 것이 맞았다.
...
이제는 적어도 운영 기준 경로가 하나의 구조로 정리되었다.
...
지금 단계에서 중요한 것은 완벽한 미래 구조를 미리 정하는 것보다,  
운영을 지속하면서 개선이 필요한 지점을 정확히 발견하는 일이다.
