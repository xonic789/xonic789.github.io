...
AWS Client VPN을 만지다 보면 `.ovpn` 파일이 자꾸 하나의 암호화된 VPN 파일처럼 느껴진다.  
그런데 이 표현은 정확하지 않다.  
`.ovpn`은 보통 암호화 그 자체를 담은 파일이 아니라, OpenVPN 클라이언트가 터널을 열기 위해 필요한 설정과 자격 증명 참조를 담은 **프로파일**에 가깝다.
...
`.ovpn`은 ZIP이나 PKCS#12 같은 독립된 암호화 컨테이너가 아니라,  
...
이 파일이 암호화되어 있느냐보다  
이 파일이 어떤 비밀 값을 직접 담고 있느냐에 더 가깝다.
...
진짜 민감한 것은 대개 이쪽이다.  
특히 `<key>...</key>`가 inline으로 들어간 `.ovpn`은 사실상 설정 파일이 아니라 자격 증명 번들에 더 가깝다.

## 예시로 보면 `.ovpn`은 이렇게 읽는다

실제로 파일을 열었을 때는 각 줄의 기능보다  
이 줄이 연결 정책인지, 공개 정보인지, 비밀 정보인지로 나눠 읽는 편이 좋다.
...
파일을 해석할 때 인증서가 있다고 안전한 것은 아니다.  
어떤 줄이 신뢰 기준을 정의하고, 어떤 줄이 실제 비밀을 담고 있는가를 먼저 봐야 한다.
...
이 구조 때문에 OpenVPN은 TLS를 쓴다는 말 하나로 끝나지 않는다.  
...
이 파일이 데이터 트래픽을 암호화한다고 표현하면 어색하다.  
...
### 1. 클라이언트는 인증서를 보여주기만 하지 않는다

클라이언트는 인증서를 전송할 뿐 아니라,  
그 인증서에 대응하는 개인 키를 정말 가지고 있다는 점도 증명해야 한다.

이 증명은 보통 TLS 핸드셰이크 안에서 일어난다.  
개인 키 자체를 보내는 것이 아니라, 핸드셰이크 메시지에 대한 서명 응답으로  
자신이 이 공개키에 대응하는 비밀키 소유자임을 증명한다.
...
OpenVPN에서 인증서 기반 mTLS만 있는 것이 아니라,  
...
중요한 것은 이 단계에서 보호되는 대상이 HTTP 요청이 아니라 우선 패킷이라는 점이다.  
...
이 파일 배포 절차가 곧 보안 경계가 된다.

이 때문에 운영 관점에서는 `.ovpn`을 설정 파일로만 다루면 안 된다.
...
인증서 기반 모델에서 흔한 착각은 잘 발급했으면 끝이라는 생각이다.  
실제로는 폐기 절차가 더 중요하다.
...
이번에 다시 정리하면서 OpenVPN과 `.ovpn`을 보는 기준이 바뀌었다.  
`.ovpn`은 대체로 평문 텍스트 프로파일이고 민감한 것은 확장자가 아니라 그 안에 포함된 개인 키와 프리셰어드 키다.  
mTLS의 본질은 개인 키 소유 증명에 있다.  
TLS 핸드셰이크가 신원 검증과 세션 키 합의를 담당하고 실제 터널 패킷은 협상된 대칭키로 보호된다.  
AWS Client VPN도 관리형 형태일 뿐, 개념적으로는 이 OpenVPN 모델 위에서 이해하는 편이 정확하다.
